-- ================================================================
-- 034_fix_duplicate_gr.sql
-- GR-2026-003 and GR-2026-004 were both confirmed against PO-2026-003.
-- GR-2026-004 (exp#32, 100,000 AZN, COGS) is the legitimate receipt.
-- GR-2026-003 (exp#44, 100,000 AZN) is a duplicate — void it.
--
-- Also rebuilds confirm_goods_receipt with an over-receipt guard:
-- if PO.status = 'received', block the confirmation so a PO can
-- never be double-received without explicit intervention.
-- ================================================================

-- ─── 1. Void the duplicate expense for GR-2026-003 ───────────────────────────

UPDATE expenses
SET    amount         = 0,
       payment_status = 'voided',
       description    = description
                        || ' [VOIDED — duplicate GR against PO-2026-003; '
                        || 'GR-2026-004/exp#32 is the valid receipt]'
WHERE  id = 44;

-- ─── 2. confirm_goods_receipt — add over-receipt guard ───────────────────────
...
BEGIN
  ...
  -- ── OVER-RECEIPT GUARD ────────────────────────────────────────────────────
  -- PO.status is set to 'received' when any GR is confirmed against it.
  -- Block a second confirmation to prevent double-billing.
  IF v_po.status = 'received' THEN
    RETURN jsonb_build_object(
      'error',
      'PO ' || v_po.po_number || ' is already fully received. '
      || 'Void or cancel the existing GR before confirming a new receipt, '
      || 'or raise a new PO for additional quantities.'
    );
  END IF;
  -- ─────────────────────────────────────────────────────────────────────────
