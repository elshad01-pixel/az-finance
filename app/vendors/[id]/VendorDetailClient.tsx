      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            {vendor.category && (
              <span className={`inline-block mt-1 text-xs font-semibold px-2.5 py-1 rounded-full ${catColor}`}>
          ...
          <Link
            ...
            className="text-xs font-medium text-gray-500 hover:text-blue-600 border border-gray-200 hover:border-blue-300 px-3 py-1.5 rounded-lg transition-colors"
          >
            {t('common.edit')}
          </Link>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
          ...
          {vendor.notes && (
            <div className="col-span-2">
              ...
              <span className="text-gray-700">{vendor.notes}</span>
            </div>
          )}
        </div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {[t('common.date'), t('common.description'), t('exp.category'), t('exp.paymentMethod'), t('exp.amountAZN')].map((h, i) => (
                    <th key={i} className={`text-xs font-semibold text-gray-500 uppercase tracking-wider px-5 py-3 ${i === 4 ? 'text-right' : 'text-left'}`}>
                      {h}
                    </th>
                  ))}
                </tr>
